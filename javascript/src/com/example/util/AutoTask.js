/**
 *
 * @param {string} packageName 所有的屏幕都需要在同一个包名下, 也就是一个app, 例如抖音，快手等，每次检查是不是一个屏幕时都需要检测包名
...
function AutoTask(packageName, globalObjects) {
    ...
    this.intervalId = -1; // 如果正在运行中的话就是大于0的数字，否则就是-1
}
...
/**
 * 启动应用并等待指定时间
 * 该方法会检查shell状态，构造启动命令并执行，同时记录日志
 * 如果shell不可用或已在运行，则返回false表示启动失败
 */
...
AutoTask.prototype.run = function () {
    ...
    // 遍历所有的屏幕，如果isScreen返回true，则执行action,不要跳出循环，一直死循环，遍历所有屏幕后再次遍历
    this.intervalId = threads.getMain().getTimer().setInterval(function () {
        ...
        for (let i = 0; i < that.screens.length; i++) {
            ...
            // 如果当前应用不是packageName对应的应用程序，则启动packageName对应的应用程序
