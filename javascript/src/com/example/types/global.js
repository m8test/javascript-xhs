 ...
 * @property {function(): boolean} isScreen - 判断是否为当前屏幕的函数
 * @property {ActionContainer} actionContainer - 如果是当前屏幕，通过此容器执行一组动作
 */
 ...
 * @property {function(): void} run - 执行动作的函数
 * @property {number} runTimes - 执行动作的次数
 * @property {number} waitTime - 执行动作后的等待时间，单位为毫秒
 */

/**
 * 动作容器，用于封装一组动作。当处于某个屏幕时，会通过该容器执行一组动作
 *
 * @typedef {Object} ActionContainer
 * @property {Array<Action>} actions - 执行的动作列表
 * @property {function(): Array<string>} getActionNames - 获取需要执行的动作名称，每次判断是某个页面时都会执行一次
 * @property {function(): void} run - 执行动作容器中的所有动作
 */
 ...
 * @property {number} viewDetailTime - 查看详情的时间，即在详情页面停留的时间，单位为毫秒
 ...
 * @property {number} commentRate - 评论的概率，0 - 100 的整数，代表评论的概率
 * @property {number} privateMessageRate - 私信的概率，0 - 100 的整数，代表私信的概率
 * @property {number} likeRate - 点赞的概率，0 - 100 的整数，代表点赞的概率
 * @property {number} favoriteRate - 收藏的概率，0 - 100 的整数，代表收藏的概率
 */
 ...
 * @property {function(string): boolean} saveConfig - 保存配置到存储中
 * @property {function(): XHSConfig} readConfig - 从存储中读取配置
 */

/**
 * 全局对象集合，包含了自动化脚本所需的各种全局对象
 *
